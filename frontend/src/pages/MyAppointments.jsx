...
const MyAppointments = () => {
    ...
    const getUserApointments = async () => {
        try {
            ...
            const {data} = await axios.get(backendUrl+'/api/user/appointments',{headers:{headers:token}})
    ...
    const initPay = (order) => {
        ...
        const rzp = new window.Pazorpay(options)
        rzp.open()
                ...
                {appointments.map((item,index)=>(
                    <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                        ...
                        <div className='flex-1 text-sm text-zinc-600'>
                            <p className='text-neutral0800 font-semibold'>{item.docData.name}</p>
                        ...
                        <div className='flex flex-col gap-2 justify-end'>
                            ...
                            {item.cancelled && <button className='sm:min-w-48 py-2 border border-red-500 rounded text-red-500'>Appoitment cancelled</button>}
                        </div>
                    </div>
                ))}
            </div>
